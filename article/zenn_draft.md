...
これがいちばん言いたいことだ。
論文シェアは研究コミュニティの関心の配分を測っている指標であって、
実務での有用性を測ってはいない。
...
むしろ手法が成熟して論文のネタでなくなった、という読み方のほうが
自然だと思う。
...
部分適用のほうが、導入を検討するなら現実的な入り口に見える。

もうひとつ気になるのは、論文で報告される改善が、
実際のオンライン指標にどれだけ乗るかである。これは LLM に限った話ではないが、
...
だとすると、arXiv で方向を掴み、会議録で選別するという
...
7年ぶんの論文を数えてわかったのは、まず LLM 関連が2024年に不連続に
立ち上がったことだ。ChatGPT 公開から会議録に現れるまで約1年半かかっており、
研究の主題転換は査読サイクル1周ぶん遅れて見える。Graph / GNN の反転も
同じ2024年に起きていて、徐々にではなく1年で起きた。ただし実数では8軸
すべて増えており、シェア低下は縮小ではなく減速である。人の動きも確認したが、
LLM 著者の6割は新規参入で、GNN からの移動（18.5%）より外部からの流入の
ほうが大きかった。

LLM 研究は既存テーマと融合してもいない。6割近くが他の軸に触れず、
グラフとの共起は 1.7%。吸収ではなく置き換えに近い。一方で
Fairness / Explainability、Reinforcement Learning、Causal / Debiasing は
7年間ほぼ横ばいだった。関心が集中していないことと、問題が解決したことは別である。
そしてプレプリントは会議録より1年以上先を走っており、いま arXiv では
推薦研究の半数以上が LLM 関連になっている。

論文の増減は研究コミュニティの関心の配分であって、
