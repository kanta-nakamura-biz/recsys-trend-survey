...
### 2.7 実数で見ると、減ったトピックはひとつも無い

シェアの議論は「相対的な関心の配分」を見ている。実数で見ると絵が変わる
（`data/processed/topic_counts_by_year.csv`）。

| トピック | 2019–2021 | 2023–2025 | 増減 | 倍率 |
|---|---:|---:|---:|---:|
| LLM-based | 2 | 173 | +171 | **86.5倍** |
| Multimodal | 23 | 94 | +71 | 4.09倍 |
| Cold-start / Federated | 72 | 186 | +114 | 2.58倍 |
| Sequential / Session-based | 112 | 247 | +135 | 2.21倍 |
| Causal / Debiasing | 49 | 93 | +44 | 1.90倍 |
| Fairness / Explainability | 61 | 96 | +35 | 1.57倍 |
| Reinforcement Learning | 53 | 79 | +26 | 1.49倍 |
| Graph / GNN | 141 | 189 | +48 | **1.34倍** |
| （推薦論文計） | 647 | 1,140 | +493 | 1.76倍 |

**8 軸すべてが実数では増えている。** 推薦論文全体が 1.76 倍に増えたので、
それより遅く伸びた軸がシェアを落としたにすぎない。
Graph / GNN の「−5.2pt」は**縮小ではなく、相対的な減速**である。

ただし年次で見ると、Graph / GNN は 2023 年の 81 本をピークに
2024 年 55 本、2025 年 53 本と**実数でも減っている**（ピーク比 65%）。
2019 年（38 本）と比べればまだ多い。
つまり「7 年で見れば増えた／直近 2 年では実数でも減った」の両方が正しい。

| 年 | 2019 | 2020 | 2021 | 2022 | 2023 | 2024 | 2025 |
|---|---:|---:|---:|---:|---:|---:|---:|
| Graph / GNN（本） | 38 | 50 | 53 | 61 | **81** | 55 | 53 |
| LLM-based（本） | 0 | 1 | 1 | 6 | 8 | 70 | 95 |
| 推薦論文計（本） | 168 | 222 | 257 | 321 | 360 | 393 | 387 |

### 2.8 「GNN の人が LLM に流れた」のか — 著者を追う

§2.2 で LLM の立ち上がりと Graph / GNN の反転が同じ 2024 年に起きていた。
人が移動したのかを、DBLP の著者 pid で追って検証した
（`src/recsys_trend/author_flow.py`）。

**移動は起きている。ただし効果は中程度。**

| 2019–2023 の群 | 2024–25 も活動 | うち LLM 論文あり | 割合 |
|---|---:|---:|---:|
| GNN 論文あり | 352 人 | 147 人 | **41.8%** |
| GNN 論文なし | 540 人 | 175 人 | 32.4% |

元 GNN 勢のほうが LLM 論文を書く率は高い（41.8% vs 32.4%）。
仮説の方向は支持される。だが差は 9.4pt であって、劇的ではない。

**主因はむしろ新規参入だった。**

| 2024–25 の LLM 著者 795 人の出自 | 人数 | 割合 |
|---|---:|---:|
| 新規参入（2019–23 に本データで論文なし） | 473 | **59.5%** |
| 既存だが GNN 以外 | 175 | 22.0% |
| 元 GNN 勢 | 147 | 18.5% |

LLM 論文の著者の 6 割は、2019–2023 にこの 6 会議で論文を書いていない。
**GNN 勢の移動より、外から入ってきた人の数のほうが大きい。**

**そして元 GNN 勢は GNN を捨てていない。**

| トピック | 元GNN勢の2024–25論文 | 2024–25全体 | 差 |
|---|---:|---:|---:|
| LLM-based | 27.0% | 24.1% | +2.9 |
| Graph / GNN | 15.4% | 12.8% | +2.6 |
| Cold-start / Federated | 15.3% | 18.0% | −2.7 |

元 GNN 勢は全体平均より LLM を書いている（+2.9pt）が、
**同時に Graph / GNN も全体平均より書いている**（+2.6pt）。
乗り換えたのではなく、**足した**という形に近い。

補足として、元 GNN 勢は 2024–25 への継続率が高い（33.2% vs 20.1%）。
グラフ研究に取り組んでいた層は相対的に定着している。

**結論**: Graph / GNN のシェア低下は「人が LLM に流出したから」だけでは
説明できない。実際に起きたのは、
(1) 新規参入者が大量に LLM 論文を書き、母数が 1.76 倍に膨らんだ、
(2) 既存の GNN 勢は一部 LLM に手を広げつつ GNN も続けている、
という構図である。**分母が増えたことの寄与が大きい。**

**限界**: 本データの 6 会議でしか著者を追えない。「新規参入」には、
他分野からの流入と、単に別の会議で活動していた著者の両方が含まれる。
両者は区別できない。
...
9. **著者分析（§2.8）は本データの 6 会議に閉じている**。他会議・ジャーナル・
   他分野での活動は見えないため、「新規参入」は「この 6 会議では新顔」の意味
   でしかない。
...
1. §2.2 の「LLM の立ち上がりと Graph の反転が同じ 2024 年に起きた」という読み方は
   ...
   変化など）で説明できる部分はないか。
1b. §2.8 の著者分析で、「LLM 著者の 6 割が新規参入」という数字をどう解釈すべきか。
   6 会議に閉じた観測であることを踏まえ、他分野からの流入と言ってよいか、
   単に観測範囲外にいた既存研究者と見るべきか。
